...
func (p *Page) SetSourceMetaData(in interface{}, mark rune) (err error) {
	// See https://github.com/zinadesign/hugo/issues/2458
	defer func() {
		if r := recover(); r != nil {
			var ok bool
			err, ok = r.(error)
...
func filterTermsInHierarchy(hierarchy map[string]interface{}, terms_defined Taxonomy, term_in_hierarchy map[string]bool) (map[string]interface{}) {
...
func outputTermsInHierarchy(term_hierarchy map[string]interface{}, taxonomy_plural string, p *Page, sort_by string, template_name string, order_direction bool) string {
	...
	if len(terms) > 0 {
		html += string(p.s.tmpl.ExecuteTemplateToHTML(terms, "partials/"+template_name, "theme/partials/"+template_name))
	}
	return html
}
